...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	*/
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if ($clipping AND !isset($attribs['fill']) AND (!isset($attribs['style']) OR (!preg_match('/[;\"\s[1]{1}fill[\s]*:[\s]*([^;\"]*)/si', $attribs['style'], $attrval)))) {
		...
		foreach ($prev_svgstyle as $key => $val) {
			...
			if (isset($attribs[$key]) AND !TCPDF_STATIC::empty_string($attribs[$key])) {
				...
				// CSS style syntax
				$attrval = array();
				if (preg_match('/[;\"\s[1]{1}'.$key.'[\s]*:[\s]*([^;\"]*)/si', $attribs['style'], $attrval) AND isset($attrval[1])) {
