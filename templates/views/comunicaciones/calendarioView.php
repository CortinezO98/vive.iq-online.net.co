...
$puedeEditar = in_array($_SESSION[APP_SESSION.'usu_perfil'] ?? '', ['ADMIN','Administrador','SUPERADMIN']);
?>
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="<?= URL ?>?uri=comunicaciones/ver/<?= $d->pagina->pag_slug ?? '' ?>"><?= $d->pagina->pag_titulo ?? 'Comunicaciones' ?></a></li>
                ...
                <div class="calendar-container">
                    <div class="calendar-header">
                        ...
                        <div class="calendar-nav">
                            <a href="?year=<?= $yearAnterior ?>&month=<?= $mesAnterior ?>" class="btn btn-outline-primary">
                                <i class="fas fa-chevron-left me-2"></i>Mes anterior
                            </a>
                            <a href="?year=<?= date('Y') ?>&month=<?= date('n') ?>" class="btn btn-outline-secondary">
                                <i class="fas fa-calendar-day me-2"></i>Hoy
                            </a>
                            <a href="?year=<?= $yearSiguiente ?>&month=<?= $mesSiguiente ?>" class="btn btn-outline-primary">
                    ...
                    <div class="calendar-grid">
                        ...
                        ?>
                            <div class="<?= $clase ?>" data-fecha="<?= $fecha->format('Y-m-d') ?>">
                                <div class="calendar-day-number">
                                    <?= $diaActual ?>
                                    <?php if ($puedeEditar && $esMesActual): ?>
                                        <button class="add-event-btn" onclick="abrirFormularioEvento('<?= $fecha->format('Y-m-d') ?>')">
                                ...
                                <div class="calendar-events">
                                    <?php foreach (array_slice($eventos, 0, 3) as $ev): ?>
                                        <div class="calendar-event <?= $ev['is_all_day'] ? 'all-day' : '' ?>" 
                                             onclick="verEvento(<?= htmlspecialchars(json_encode($ev)) ?>)"
                                             style="border-left-color: <?= $ev['color'] ?? '#1C2262' ?>;">
                                            <?php if (!$ev['is_all_day'] && $ev['start_time']): ?>
                                                <div class="event-time">
                                                    <?= substr($ev['start_time'], 0, 5) ?>
                                                    <?php if ($ev['end_time']): ?> - <?= substr($ev['end_time'], 0, 5) ?><?php endif; ?>
                                                </div>
                                            <?php else: ?>
                                            ...
                                            <?php if ($ev['meet_url']): ?>
                                                <a href="<?= htmlspecialchars($ev['meet_url']) ?>" target="_blank" class="event-link" onclick="event.stopPropagation()">
        ...
        <div class="modal-content">
            ...
            <form id="eventoForm">
                <div class="modal-body">
                    ...
                    <input type="hidden" name="token" value="<?= $_SESSION['iqvive_token'] ?>">
...
function verEvento(evento) {
    ...
    if (!evento.is_all_day && evento.start_time) {
        html += `<br>${evento.start_time.substring(0,5)}`;
        if (evento.end_time) {
            html += ` - ${evento.end_time.substring(0,5)}`;
        }
    } else {
        html += `<br>Todo el día`;
    }
    ...
    if (evento.meet_url) {
        html += `
            <div class="event-detail">
                ...
                <div class="event-detail-value">
                    <a href="${evento.meet_url}" target="_blank">${evento.meet_url}</a>
                </div>
            </div>
        `;
    }
...
// Validar horas cuando se desmarca "Todo el día"
